...
class LLMModel:
    def __init__(self, args):
        ...
        self.model_name, self.model_version = read_model_name(args.engine_dir)
        if args.tokenizer_dir is None:
            logger.warning(
                "tokenizer_dir is not specified. Try to infer from model_name, but this may be incorrect."
            )
            args.tokenizer_dir = DEFAULT_HF_MODEL_DIRS[self.model_name]
...
def main(llm_model, runner, input_text, args):
    ...
    pad_id = llm_model.pad_id
    end_id = llm_model.end_id
